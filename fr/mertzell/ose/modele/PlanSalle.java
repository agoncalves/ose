package fr.mertzell.ose.modele;
...
public class PlanSalle implements Iterable<PlanSalle.Poste> {
	...
	public void modifierPoste(Position position,int orientation){
		//*************Version avec un for*****************
		//for(Poste lePoste : lesPostes){
			//if(position.equals(lePoste.getPosition())){
				//lePoste.setOrientation(orientation);
			//}
		//}
		//**********************************************
		 ...
		 int test = 0 ;
		     do {
		        if(position.equals(lesPostes.get(count).getPosition())){
					lesPostes.get(count).setOrientation(orientation);
					test = 1;
		        }
		        count++;
	...
	public Iterator<PlanSalle.Poste> iterator(){
